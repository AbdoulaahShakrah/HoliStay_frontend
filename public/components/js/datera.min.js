(() => {
    const Datera = function (
        ...
        if (options.disabledDates) {
            this.disabledDates = options.disabledDates?.map(
                (val) => new Date(val)
            );
        }
        if (options.enabledDates) {
            this.enabledDates = options.enabledDates?.map(
                (val) => new Date(val)
            );
        }
    ...
    Datera.prototype = {
        ...
        placeCalendar() {
            if (this.calendarDOM) {
                const calendarBoundingRect =
                    this.calendarDOM.getBoundingClientRect();
                const inputBoundingRect = this.input.getBoundingClientRect();

                // Posiciona o calendário diretamente abaixo do input
        ...
        getCalendarDays() {
            ...
            fromStartOfMonth.setDate(
                1 + this.startWeek - selectedMonth.getDay()
            );
            if (
          ...
          <button class="datera--month-increment" ${
            ...
            this.calendarDOM
                ...
                .addEventListener("change", () => {
                    this.currentDate.setFullYear(
                        this.calendarDOM.querySelector(".datera--year-select")
                            .value
                    );
                    this.showCalendar();
                });
            this.calendarDOM
                ...
                .addEventListener("change", () => {
                    this.currentDate.setMonth(
                        this.calendarDOM.querySelector(".datera--month-select")
                            .value
                    );
                    this.showCalendar();
                });
        ...
        onDatePicked(selectionDate) {
            ...
            if (this.selectionType == "range") {
                if (!this.value[0]) {
                    ...
                    if (
                        this.value[0]?.toDateString() !=
                        decodedSelectionDate.toDateString()
                    ) {
